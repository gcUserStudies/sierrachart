...
//
// 05/05/2023 - Compiled using Sierra Chart: 2499
//
...
SCSFExport scsf_gcNoMoPaperHands(SCStudyInterfaceRef sc)
{
	...
	t_OrderQuantity32_64 Quantity = PositionData.PositionQuantity; // Access the quantity
...
SCSFExport scsf_gcAccountBalanceExternalService(SCStudyInterfaceRef sc)
{
	...
	if (sc.GetTradeAccountData(TradeAccountDataFields, TradeAccount))
	{
		...
		if (Input_SelectAccountBalance.GetIndex() == 1)
			AccountBalance = TradeAccountDataFields.m_AvailableFundsForNewPositions;
...
void MBODrawToChart(HWND WindowHandle, HDC DeviceContext, SCStudyInterfaceRef sc)
{
	...
	for (int LevelIndex = 0; LevelIndex < MBOLevels; LevelIndex++)
	{
		...
		int ActualLevelsBid = sc.GetBidMarketLimitOrdersForPrice(sc.Round(MarketDepthEntryBid.Price / sc.TickSize), NumberOfMarketOrderDataElements, MarketOrderDataBid);
		...
		y_BidDOMColumn = sc.RegionValueToYPixelCoordinate(MarketDepthEntryBid.Price, sc.GraphRegion);
	...
	for (int LevelIndex = 0; LevelIndex < MBOLevels; LevelIndex++)
	{
		...
		int ActualLevelsAsk = sc.GetAskMarketLimitOrdersForPrice(sc.Round(MarketDepthEntryAsk.Price / sc.TickSize), NumberOfMarketOrderDataElements, MarketOrderDataAsk);

		// Show fewer elements if user selects smaller number
		ActualLevelsAsk = min(ActualLevelsAsk, MBOElements);

		// Determine Vert Position and Offset for GP1 / GP2 columns
		y_AskDOMColumn = sc.RegionValueToYPixelCoordinate(MarketDepthEntryAsk.Price, sc.GraphRegion);
